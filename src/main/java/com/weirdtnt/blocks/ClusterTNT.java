...
import com.weirdtnt.entities.ClusterTntEntity;
...
public class ClusterTNT extends TntBlock {
    public ClusterTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            ClusterTntEntity entity = new ClusterTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
