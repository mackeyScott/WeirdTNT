...
import com.weirdtnt.entities.AttractorTntEntity;
...
public class AttractorTNT extends TntBlock {
    public AttractorTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            AttractorTntEntity entity = new AttractorTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
