...
import com.weirdtnt.entities.BookTntEntity;
...
public class BookTNT extends TntBlock {
    public BookTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            BookTntEntity entity = new BookTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
