...
import com.weirdtnt.entities.RepulsorTntEntity;
...
public class RepulsorTNT extends TntBlock {
    public RepulsorTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            RepulsorTntEntity entity = new RepulsorTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
