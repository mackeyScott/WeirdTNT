...
import com.weirdtnt.entities.BreachTntEntity;
import com.weirdtnt.entities.RainbowTntEntity;
...
public class RainbowTNT extends TntBlock {
    public RainbowTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            RainbowTntEntity entity = new RainbowTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
