...
import com.weirdtnt.entities.RandomiseTntEntity;
...
public class RandomiseTNT extends TntBlock {
    public RandomiseTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            RandomiseTntEntity entity = new RandomiseTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
