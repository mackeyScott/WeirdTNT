...
import com.weirdtnt.entities.RestoreTntEntity;
...
public class RestoreTNT extends TntBlock {
    public RestoreTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            RestoreTntEntity entity = new RestoreTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
