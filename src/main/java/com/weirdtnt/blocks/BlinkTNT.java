...
import com.weirdtnt.entities.BlinkTntEntity;
...
public class BlinkTNT extends TntBlock {
    public BlinkTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            BlinkTntEntity entity = new BlinkTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
