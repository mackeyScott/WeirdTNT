...
import com.weirdtnt.entities.LightningTntEntity;
...
public class LightningTNT extends TntBlock {
    public LightningTNT(Settings settings) {
        super(settings);
    }
    ...
    private void ignite(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            LightningTntEntity entity = new LightningTntEntity(world,
                    pos.getX() + 0.5,
                    pos.getY(),
                    pos.getZ() + 0.5,
                    igniter);
