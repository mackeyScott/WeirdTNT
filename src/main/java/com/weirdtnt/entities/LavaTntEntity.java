...
public class LavaTntEntity extends TntEntity {
    ...
    public LavaTntEntity(EntityType<? extends TntEntity> type, World world) {
        super(type, world);
    }
            ...
            for (int dy = -radius; dy <= radius; dy++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (target.getManhattanDistance(center) <= radius) {
                        this.getWorld().setBlockState(target, Blocks.LAVA.getDefaultState(), 3);
